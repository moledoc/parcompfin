...
prep_data <- function(what){
	...
	data[,Parallel_cor:=if_else(as.integer(Parallel/1000)==1 | Parallel%%1000==1,as.integer(Parallel/1000)*Parallel%%1000,as.integer(Parallel/1000)+Parallel%%1000)]
	data[,Parallel_cor:=max(1,Parallel_cor)]
	...
	data[,Efficiency_overall:=RSpeedup_overall/Parallel_cor]
	data[,Efficiency_calculation:=RSpeedup_calculation/Parallel_cor]
...
line_graph(mc_amer,"Efficiency_overall","Efektiivsus","Efektiivsus programmil","MC simulatsioon Ameerika optsioonil",T,m=100,e=75,"call")
```
