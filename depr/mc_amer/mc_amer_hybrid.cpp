...
/* #pragma omp parallel for */
  for(int n=0;n<N/2;++n){
    ...
    gen.seed(time(&cur_time)+(n+1)*(rank+1+thread));
    // init new path
    paths(0,n) = S0;
    paths(0,n+N/2) = S0;
    // fill path
 ...
 )
{
  ...
  if(N_p%2!=0) ++N_p;
  /* if(N_p%size!=0) N_p_t = (N_p+threads-N_p%threads)/threads; */
  /* else N_p_t = N_p/threads; */
  /* if(N_p_t%2!=0) ++N_p_t; */
  double N_fixed=N_p*size;
  /* // calculate paths */
  Eigen::MatrixXd paths = pathsfinder(S0,E,r,sigma,T,N_p,M,rank,omp_get_thread_num());
  /* Eigen::MatrixXd paths(M+1,N_p_t);// = pathsfinder(S0,E,r,sigma,T,N_p,M,rank); */
...
/*     paths = pathsfinder(S0,E,r,sigma,T,N_p_t,M,rank,omp_get_thread_num()); */
/*   }; */
/*   } */
  ...
  if(rank==0) result/(double)N_fixed;
  else return 0;
}
