...
/* #pragma omp parallel for */
  for(int n=0;n<N/2;++n){
    ...
    gen.seed(time(&cur_time)+(n+1)*(rank+thread+1));
    // init new path
    paths(0,n) = S0;
    paths(0,n+N/2) = S0;
    // fill path
 ...
 )
{
  ...
  int N_p_t;
  if(N_p%threads!=0) N_p_t=(N_p+threads-N_p%threads)/threads;
  else N_p_t=N_p/threads;
  if (N_p_t%2!=0) ++N_p_t;
   
  double N_fixed=N_p*size;

  Eigen::MatrixXd paths(M+1,N_p_t);
...
#pragma omp for reduction(merge:paths) schedule(dynamic,1)  //nowait
  for(int i=0;i<threads;++i){
    paths = pathsfinder(S0,E,r,sigma,T,N_p_t,M,omp_get_thread_num());
  };
  }
  /* // calculate paths */
  /* Eigen::MatrixXd paths = pathsfinder(S0,E,r,sigma,T,N_p,M,rank,thread); */
